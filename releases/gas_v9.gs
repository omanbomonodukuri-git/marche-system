...
function handleSaveOrderFast(data) {
  ...
  if (itemRows.length > 0) iSh.getRange(iSh.getLastRow()+1,1,itemRows.length,13).setValues(itemRows);
  ...
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][0] === p.id) {
      sh.getRange(i+1,1,1,8).setValues([[
        p.id, p.name, p.price, p.totalMinutes, json, p.stock||0, p.stockWarn||3, typesJson
      ]]);
      return ok({ saved: true });
    }
  }
  sh.appendRow([p.id, p.name, p.price, p.totalMinutes, json, p.stock||0, p.stockWarn||3, typesJson]);
  return ok({ saved: true });
}
...
function setup() {
  ...
  createSheetWithHeaders(ss, SH.PRODUCTS,  ['id','name','price','totalMinutes','stepTimesJson','stock','stockWarn','typesJson']);
  createSheetWithHeaders(ss, SH.ORDERS,    ['id','num','note','deliveryType','createdAt','completedAt','status','sharedImageRef','channel']);
  createSheetWithHeaders(ss, SH.ITEMS,     ['id','orderId','pid','idx','totalOf','skipBinarize','skipDesign','price','paymentMethod','onHold','paid','typeId','typeName']);
...
function fixAllSheets() {
  ...
  fixHeader(ss,'items',    ['id','orderId','pid','idx','totalOf','skipBinarize','skipDesign','price','paymentMethod','onHold','paid','typeId','typeName']);
  fixHeader(ss,'steps',    ['id','itemId','stepIndex','done','startedAt','completedAt','durationMins']);
  fixHeader(ss,'products', ['id','name','price','totalMinutes','stepTimesJson','stock','stockWarn','typesJson']);
