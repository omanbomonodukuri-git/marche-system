...
function handleUpdateStep(data) {
  ...
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][0] === data.stepId) {
      ...
      // startedAt：渡された値 or 既存値 or completedAtと同じ（最低でも設定）
      var startedAt = data.startedAt || rows[i][4] || data.completedAt || '';
      if (startedAt) sh.getRange(i+1,5).setValue(startedAt);

      if (data.completedAt) {
        sh.getRange(i+1,6).setValue(data.completedAt);
        // 実績時間計算：startedAt が有効な場合のみ
        var start = new Date(startedAt);
        var end   = new Date(data.completedAt);
        if (startedAt && !isNaN(start.getTime()) && !isNaN(end.getTime())) {
